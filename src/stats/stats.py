...
from utilities.stats.save_plots import save_plot_from_data, \
    save_pareto_fitness_plot, save_plot_from_data_tsc
...
def get_soo_stats(individuals, end):
    ...
    if params['SAVE_PLOTS'] and not params['DEBUG']:
        if not end:
            ...
            trackers.best_fitness_list_auroc.append(trackers.best_ever.fitness_auroc)
            trackers.best_fitness_list_Pearson_corr.append(trackers.best_ever.fitness_Pearson_corr)

        if params['VERBOSE'] or end:
            save_plot_from_data(trackers.best_fitness_list, "best_fitness")
            save_plot_from_data(trackers.best_fitness_list_auroc, "best_fitness_auroc")
            save_plot_from_data(trackers.best_fitness_list_Pearson_corr, "best_fitness_Pearson_corr")
            save_plot_from_data_tsc(trackers.best_fitness_list, trackers.best_fitness_list_auroc, trackers.best_fitness_list_Pearson_corr)
...
def update_stats(individuals, end):
    ...
    # TIME SERIES CLASSIFICATION STATS
    auroc = [i.fitness_auroc for i in individuals]
    stats['ave_auroc'] = np.nanmean(auroc, axis=0)
    stats['best_auroc'] = trackers.best_ever.fitness_auroc

    Pearson_corr = [i.fitness_Pearson_corr for i in individuals]
    stats['ave_Pearson_corr'] = np.nanmean(Pearson_corr, axis=0)
    stats['best_Pearson_corr'] = trackers.best_ever.fitness_Pearson_corr
