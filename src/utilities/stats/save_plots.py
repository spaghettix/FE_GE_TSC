...
def save_plot_from_data_tsc(fitness, fitness_auroc, fitness_Pearson_corr):
    """
    Saves a plot of a given set of data.
    ...
    # Plot data.
    ax.plot(fitness, 'r', label='Fitness')
    ax.plot(fitness_auroc, 'g', label='AUROC')
    ax.plot(fitness_Pearson_corr, 'b', label='Pearson corr.')
    ...
    # Plot title.
    plt.title(f"{params['DATASET_NAME']} - Feature {params['FEATURE_NUMBER']} - Class {params['CLASS']}")
    plt.legend()

    # Save plot and close.
    plt.savefig(path.join(params['FILE_PATH'], f"{params['DATASET_NAME']}_{params['CLASS']}_{params['FEATURE_NUMBER']}.pdf"),
                bbox_inches='tight')
    plt.close()
